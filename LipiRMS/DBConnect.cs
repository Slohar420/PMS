...
namespace Classes
{
    public class DBConnect : IDisposable
    {
        ...
        private void Initialize()
        {
            ...
            LipiRMS.Log.Write(connectionString, "ServiceDBLogs");
        ...
        private bool OpenConnection(out string strError)
        {
            ...
            try
            {
                if (database_type == DBType.mssql)
                {
                    ...
                    {
                        LipiRMS.Log.Write("try to open connection", "ServiceDBLogs");
                        connection.Open();
                        LipiRMS.Log.Write("Connection Opended ", "ServiceDBLogs");
            ...
            {
                LipiRMS.Log.Write("error in connection open", "ServiceDBLogs");
        ...
        private bool CloseConnection(out string strError)
        {
            ...
            try
            {
                if (database_type == DBType.mssql)
                {
                    ...
                    {
                        LipiRMS.Log.Write("try to close connection", "ServiceDBLogs");
                        connection.Close();
                ...
                else
                {
                    if (myconnection.State == ConnectionState.Open)
                    {
                        LipiRMS.Log.Write("try to close connection", "ServiceDBLogs");
                        myconnection.Close();
                        LipiRMS.Log.Write(" connection Closed", "ServiceDBLogs");
                        //ThreadSafe.WriteFile("S", "Database connection :", "Connection Close", "MainFolder");
                    }
                    return true;
                }
            ...
            catch (SqlException ex)
            {
                ...
                LipiRMS.Log.Write("errer in close connection", "ServiceDBLogs");
                strError = ex.Message;
                return false;
            }
        }
